from fastapi import FastAPI, Response, APIRouter, File, UploadFile, Form, HTTPException, Depends, status
...
class API:
    ...
    def setup_routes(self):
        ...
        async def validate_credentials(request: LoginRequestModel, response: Response) -> LoginResponseModel:
        ...
        @self.router.get("/protected_route")
        async def protected_route(current_user: dict = Depends(get_current_user)):
            # Log the current user's username (or any relevant data) for demonstration
            self.logger.info(f"Accessing protected route, Current User: {current_user['username']}")

            # Use 'current_user' data in your route logic or response as needed
            return {"message": "You are accessing a protected route", "user": current_user['username']}
        ...
        #     # You can use 'current_user' to access the user's information from the token
